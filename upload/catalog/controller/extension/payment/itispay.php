...
require_once(DIR_SYSTEM . 'library/itispay/version.php');
...
class ControllerExtensionPaymentItispay extends Controller
{
    ...
    public function index()
    {
        ...
        $data['action'] = $this->url->link('extension/payment/itispay/checkout', '', true);

        return $this->load->view('extension/payment/itispay', $data);
    }
    ...
    public function checkout()
    {
        ...
        $request = array(
            ...
            'cancel_url' => $this->url->link('extension/payment/itispay/callback', '', true),
            'callback_url' => $this->url->link('extension/payment/itispay/callback', '', true),
            'success_url' => $this->url->link('extension/payment/itispay/success', '', true),
            'email' => $order_info['email'],
            'plugin' => 'opencart',
            'version' => ITISPAY_OPENCART_EXTENSION_VERSION
        );
        ...
        if ($response && $response['status'] !== 'error' && !empty($response['data'])) {
            ...
            $this->model_extension_payment_itispay->addOrder($orderData);
            $this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get('payment_itispay_order_status_id'));
            $this->cart->clear();
            $this->session->data['fail'] = false;
            if (isset($shop['data']['white_label']) && $shop['data']['white_label']) {
                $this->response->redirect($this->url->link('extension/payment/itispay/invoice', '', true));
            } else {
    ...
    public function invoice()
    {
        ...
        $itispayOrder = $this->model_extension_payment_itispay->getOrder($orderId);
        if (!$itispayOrder){
            $this->response->redirect($this->url->link('common/home', '', true));
        }
        ...
        $this->response->setOutput($this->load->view('extension/payment/itispay_invoice', $data));
    }
    ...
    public function success()
    {
        if (isset($this->session->data['order_id'])) {
            ...
            $order = $this->model_extension_payment_itispay->getOrder($this->session->data['order_id']);
        } else {
            $order = '';
        }
    ...
    public function callback()
    {
        ...
        if ($this->verifyCallbackData($this->request->post)) {
            ...
            if (!empty($order_info)) {
                $ext_order = $this->model_extension_payment_itispay->getOrder($order_id);
                if (!empty($ext_order) && isset($ext_order['wallet_hash']) && !empty($ext_order['wallet_hash'])) {
                    ...
                    $this->model_extension_payment_itispay->updateOrder($data);
                }
    ...
    private function setupClient()
    {
        ...
        $this->secretKey = $this->model_setting_setting->getSettingValue('payment_itispay_api_secret_key');
        $this->client = new Client($this->secretKey);
    }
}
